...
: World( World ), IsActive( true )
{
   ...
   // mtl->setSceneBlending( Ogre::SBT_TRANSPARENT_COLOUR);
   // mtl->setDepthBias( 0.1, 0);
 //   mtl->setDepthWriteEnabled(0);
    mtl->setDepthFunction(Ogre::CMPF_ALWAYS_PASS);
 //   mtl->setDepthCheckEnabled(0);
   ...
   // ASSERT( tu );
    ...
    mtl->getTechnique(0)->setSelfIllumination( Ogre::ColourValue::Green );

    DebugLineDrawer->setMaterial(matName);

    DebugLineDrawer->setRenderQueueGroup(Ogre::RENDER_QUEUE_OVERLAY-1);
